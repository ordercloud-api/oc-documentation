...
- `GET v1/me/sellers`
   ...
   - IDs of results represnt valid values for setting Order.ToCompanyID and can be used in product queries (below).

#### New optional query parameter for getting Seller-specific view of product(s):
- `GET v1/me/products?sellerID={sellerID}`
   - Acts as a filter and to return Seller-specific pricing, if applicable.
