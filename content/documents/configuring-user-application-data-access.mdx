...
Let's make a request to assign our `defaultbuyer` security profile to our buyer copmany with ID `buyercompany`. Once we have successfully made this assignment, these roles will cascade down through any user under this buyer company. If successful, you should receive a `204 Created` response.
<CodeExample
  content={{
