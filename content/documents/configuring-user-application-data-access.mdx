...
Let's make a request to assign our `defaultbuyer` security profile to our buyer user with ID `buyeruser`.  If successful, you should receive a `204 Created` response.
<CodeExample
  content={{
...
Next, let's make a request to assign our `defaultseller` security profile to our sellers.  To assign a security profile to all seller users, the only thing we specify in the request body is the ID of the security profile.  If successful, you should receive a `204 Created` response.
<CodeExample
  content={{
    http: `POST https://sandboxapi.ordercloud.io/v1/securityprofiles/assignments HTTP/1.1
Authorization: Bearer INSERT_ACCESS_TOKEN_HERE
