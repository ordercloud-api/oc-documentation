...
The `Vercel-Storefront-Connector` client is for buyer users. It is used to authenticate requests to ordercloud that render the storefront like the PLP, PDP, Cart, Checkout, etc. It is given limitted API permissions - for example, you can't use it to change product features or read orders accross users. These would be inappropriate things for a buyer to do. Because its permissions are limitted, it is safe to expose the details to a client browser. It is the safer, more common method, being used to make the vast majority of requests to OrderCloud. The `Vercel-Middleware-Connector` client, on the other hand, is given full API access. It can read or modify any piecce of data in the OrderCloud marketplace. Accordingly, it also requires a Client Secret which should never be public. It is only used inside serverless functions which do pre-canned actions after appropriate security checks. In this app it is used in only 1 place, to set Payment.Accepted to true after user's credit card details are considered captured. 

Hopefully this explains why 2 API Clients are needed by this integration.   
