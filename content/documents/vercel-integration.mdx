...
If you select the "Seed a new marketplace" option of the integration, a new marketplace will be created and seeding with this [data template](https://github.com/ordercloud-api/ordercloud-seed/blob/main/seeds/Vercel-B2C.yml). Since this template already contains API Clients with names `Vercel-Middleware-Connector` and `Vercel-Storefront-Connector`, they will not be overwritten. Everything will "just work". However, if you choose to connect to another storefront, you will have to make sure these newly created API Clients are connected to organizations and default users you desire.
...
The integration "just works" when the matching storefront code and seed template are used. However, it could be set up to connect a custom storefront with a custom ordercloud configuration. 