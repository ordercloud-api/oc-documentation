...

{
    ...
    "CustomErrorUrl": "https://my-buyer-application.com/error?ErrorMessage={0}" // optional redirect to direct to a custom url upon error
}
```
...
```
The error message `ErrorMessage={0}` will be replaced with the error text from OrderCloud. 