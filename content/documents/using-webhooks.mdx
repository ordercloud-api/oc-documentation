...
<video playsinline loop autoplay muted preload="auto" style="max-width:100%;min-height:350px;">
  <source src="/images/knowledge-base/inline/using-webhooks/creating-a-new-webhook.mp4" type="video/mp4"/>
  <img src="/images/knowledge-base/inline/using-webhooks/create-webhook.png" type="img"/>
</video>
...
Elevated roles relay an `access_token` with higher permissions than the client that triggered the webhook. For example, if an authenticated user only had the UserAdmin role but our automated workflow needed to list Categories and perform some other action, we would add CategoryReader to the elevated claims to grant our endpoint a token and simplify our logic. For this example we will choose FullAccess here and save the webhook.
