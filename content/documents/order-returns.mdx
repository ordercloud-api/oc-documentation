...
- An `OrderReturn` is not required to have any `ItemsToReturn`, and `ItemsToReturn` are not required to have a `Quantity` specified 
   ...
   - Examples include refunding expedited shipping charges due to processing delays or providing a partial discount on an item that arrived damaged 
...
```
**Notes:**
- The endpoint the Integration Event will `POST` the payload to the `CustomImplementationUrl` is `/calculateorderreturn`
