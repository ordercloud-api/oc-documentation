...
These features are now available in a UAT region in the Sitecore OrderCloud Portal. If you would like access to test out these features and provide feedback please reach out in Sitecore Community Slack #ordercloud-support channel. You can register for <Link to="https://docs.google.com/forms/d/1bAVDgP5-FhFh8ohPchHtifq-rz7EBkuPojAzdEofJyo/viewform?edit_requested=true">Slack</Link> here if you haven’t already.
...
```
The `Locale` resource is assignable to Buyers and Buyer User Groups. Both `Currency` and `Language` are string properties with a maximum length of 100. We recommend using <Link to="https://en.wikipedia.org/wiki/ISO_4217">ISO-4217</Link> currency codes and <Link to="https://en.wikipedia.org/wiki/ISO_639-1">ISO-639-1</Link> language code - <Link to="https://en.wikipedia.org/wiki/List_of_ISO_3166_country_codes">ISO-3166</Link> Country Codes respectively for compatibility with other platforms such as payment processors, tax calculation services, and content management systems. The ID property is writable and follows the same convention as all other <Link to="https://ordercloud.io/learn/ordercloud-basics/conventions#writeable-ids">writable IDs</Link> in OrderCloud. 
