---
type:           article
title:          Understanding OrderCloud Assignments
description:    When you save an assignment you are creating a relationship between a party and an object (OrderCloud "thing" such as product, category, etc). For example you might assign a usergroup to a category, thereby granting a user in that group visibility to that category.
author:         tmenier
...
Resources can be assigned to many different parties. Those parties can be assigned to many other resources. For example, one user can be assigned to multiple addresses while one address can be assigned to multiple users.

![Assignments](/images/knowledge-base/inline/assignments/oc-docs__assignments-1.png)
