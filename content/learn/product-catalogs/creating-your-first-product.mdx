...
For our streamlined example we would like to create a purshase-able product in the easiest way possible. That means we will want to take advantage of the `DefaultPriceSchedule` field.

First, let's create the product. Beacuse we do not have a price schedule yet, we will leave that field out for now.

<CodeExample
  title="Create a new product"
  content={{
    http: `POST https://api.ordercloud.io/v1/products HTTP/1.1
...
Product response = await _oc.Products.CreateAsync(data);`}}
/>
...
<ContentLink to="/knowledge-base/price-schedules" subtitle="Further Reading">OrderCloud Price Schedules</ContentLink>
...
<CodeExample
  ...
  content={{
    http: `POST https://api.ordercloud.io/v1/priceschedules HTTP/1.1
Authentication: Bearer INSERT_ACCESS_TOKEN_HERE
Content-Type: application/json; charset=UTF-8;\n
{
    ID: "SHIRT_PRICE",
...
PriceSchedule response = await _oc.PriceSchedules.CreateAsync(data);`}}
/>

<CodeExample
  title="Update the default price schedule ID"
  content={{
    http: `PATCH https://api.ordercloud.io/v1/products/SHIRT HTTP/1.1
Authentication: Bearer INSERT_ACCESS_TOKEN_HERE
Content-Type: application/json; charset=UTF-8;\n
{
...
csharp: `using OrderClould.SDK;\n
var client = new OrderCloudClient(...);\n
var data = new PartialProduct {
    DefaultPriceScheduleID: "SHIRT_PRICE"
};\n
Product response = await _oc.Products.PatchAsync("SHIRT", data);`}}
/>
