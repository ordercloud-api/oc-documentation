...
Now that you've submitted your first order as a buyer user let's take a look at how an admin user can look at that same order and take action on it like completing it. In order to do that you'll first need to understand a very important concept in OrderCloud - the order "direction". 
...
An order goes from the buyer **to** the seller to fulfill. From the perspective of the buyer user that order is *outgoing*, and conversely from the perspective of the seller that order is *incoming*. Let's see this in action. First we'll want to switch perspectives to our admin user
