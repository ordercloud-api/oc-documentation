...
OrderCloud is a RESTful API that enables you to create complex, custom B2B eCommerce solutions.

This guide is meant to get you started with OrderCloud as quickly as possible. In order to accomplish this, we’ve stripped away some of the detail, so keep an eye out for links to relevant guides that provide much more in-depth explanations.

## Creating an Account

The first thing you will need to do is [create an account](https://developer.ordercloud.io/register). Registration is free and gives you access to the entire OrderCloud platform.
...
Now that you have an OrderCloud account you can navigate to your [dashboard](https://dashboard.ordercloud.io) and access your first <Link to="/getting-started/introduction-to-ordercloud">seller organization</Link>.

It is the highest level container that encompasses everything else in OrderCloud (applications, products, pricing, users, etc.). You can have multiple Seller Organizations; however, data is not shared between them.
...
Using the API Console is the simplest way to start exploring OrderCloud; however, understanding the fundamentals of how to access and use the API on it's own is extremely important for any OrderCloud developer.

## Authentication

Before you are able to interact with the OrderCloud API you will need to get an `access_token` from the auth server. The following sections will include raw request and response examples to the OAuth API.

The first thing you need is the `ClientID`. This unique identifier represents a single application on OrderCloud. Feel free to use the ClientID from the "Seller App" API Client that was automatically created on your first Seller Organization.

![API Settings](/images/docs/getting-started/quick-start-guide/api-settings.png)
