...
![Language Agnostic](/images/docs/getting-started/introduction-to-ordercloud/agnostic.jpg)

OrderCloud enables developers to use their tech stack of choice. Plus, releases are automatic and non-breaking; there’s a clear, inherent boundary between our code and yours. Nearly all modern web and mobile front-end frameworks are designed to work well out of the box with JSON-over-HTTP services that follow RESTful patterns.

![Scalling](/images/docs/getting-started/introduction-to-ordercloud/scale.jpg)
...
OrderCloud plays nicely with other similarly architected services. We are entering the age of [microservices](http://martinfowler.com/articles/microservices.html), where best-of-breed solutions are not built from scratch. Rather, they are assembled from the best parts. Our "part" is B2B eCommerce and Order Management, a complex domain in which we have been building and refining an incredibly rich and flexible data model since 1999.

## How OrderCloud Supports Complex B2B eCommerce
...
OrderCloud provides rich support for modeling a typical B2B buyer organization. Products can be created once and made available for purchase to any number of buyers, but details such as pricing, inventory, specs, and other configuration details can be customized per buyer according to their specific contract.

### Organization Hierarchy, Roles, and Visibility
...
OrderCloud provides the ability to accurately model:
...
OrderCloud’s built-in rules engine allows you to express these rules as formulas that get evaluated each time an order is submitted or approved. When an approval triggers no further rules, the order is passed on to the seller.
...
OrderCloud provides a clear method of impersonating a buyer user. This allows the CSR to see exactly what that specific user sees and place orders on their behalf, while protecting sensitive information such as passwords.
...
As a RESTful API, OrderCloud lends itself equally well to back-end integrations as it does front-end apps. Webhooks are a powerful mechanism that allow middleware platforms and custom server applications to subscribe to events (also via HTTP/JSON but in the reverse direction) that occur within OrderCloud, such as order submitted, item shipped, product changed, etc..

## Feature Rich API

The OrderCloud API has some standard conventions that permeate the entire platform. You can expect these features to behave the same no matter which resource you're working with.
...
<section id="feature-rich" style="display: inline-flex">
  ...
  <div style="max-width: 30%;">
    <h3>OAuth 2.0</h3>
    <p>
      OrderCloud API authentication is based on the OAuth 2 specification and
...
Companies of all types and sizes rely on OrderCloud for automating a wide variety of business operations. Now is the time to future-proof your technology with an infrastructure that will last the test of time. Creating an account is free and outlined in the next article.