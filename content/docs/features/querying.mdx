...
You can also optionally specify which fields (of the searchable fields available) are searched by providing a `searchOn` parameter for any field available on the resource you're searching (including xp). Here's we're searching on Name and xp.Color so if our search finds a match in either of those two fields we will get a result.
