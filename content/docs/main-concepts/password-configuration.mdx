---
title: 'Custom Password Config'
section: 'Main Concepts'
priority: 7
---

# Configuring Custom Password Security settings
...
The release of the new feature will be followed by an OrderCloud-wide enforcement of a [more secure password policy](#new-minimum-password-requirements) than the one that exists today. This is a planned change that encompasses the latest recommendations from security industry experts. ***This change may impact your users; it is important that you implement password configurations and test your applications against the forthcoming policy to make any necessary updates to your code prior to enforcement on May 10, 2021.***
...
-   **`LimitPasswordReuse`** (int?) - The number of previously used passwords restricted from reuse. If set to 3, the user would not be able to reset their password to any of the previous 3 passwords they have used.
...
The security profile created in the snippet below meets all of the new minimum password security requirements that will be enforced beginning May 10, 2021. 
```json
...
```
Before you assign your new profile to an organization, group, or user, make sure you have a user created with a password that does not meet the new requirements. Once you have that information attempt to authenticate as that user, and you will receive `400` error back from OrderCloud with information on this specific user's minimum password requirements. You'll need to handle this error and display the information to the user and direct them to go through the normal password reset flow.
```json
{
	"error": "invalid_grant",
	"error_description": "Password does not meet security requirements."
	....need to add further error details here when Jeff finalizes...
}
```
...
There is no limit on the number of security profiles with password configurations you can have. Be sure to check out the [section below](#things-to-note) for information on what happens if a user is assigned multiple security profiles with different password configurations.
...
## New Minimum Password Requirements
 ...
 Beginning on May 10, 2021 OrderCloud will enforce the following minimum password requirements for all users in all applications:
...
-   10 allowed failed authentication attempts followed by a 10 minute lockout period
