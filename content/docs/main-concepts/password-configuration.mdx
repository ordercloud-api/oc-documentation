...
Every [security profile](https://ordercloud.io/main-concepts/security-profiles) has a PasswordConfig object available on it. In order to enforce a minimum password configuration for all users assigned to that security profile, you just need to populate the values with your desired password policy.
